...
import { parseWorldCovidData, worldData } from '../utils/csvUtil'
...
export default {
  ...
  methods: {
    ...
    queryBasicData (date) {
      // 从 worldData 中找到对应日期的数据
      const dateData = worldData.find(item => item.date === date);
    ...
    queryProvinceDataList (date) {
      ...
      if (dateData) {
        ...
        console.log('mystd ', provinceData)
    ...
    setProvinceRankingData (areaList = []) {
      ...
      if (areaList.length !== 0) {
        for (let i = 0; i < Math.min(areaList.length, 10); i++) {
    ...
    initAllChart() {
      ...
      this.$refs.dataWorldMap.initChart()
    }
  },
  async mounted() {
    this.initAllChart()
    ...
    // 等待 CSV 数据加载完成
    await parseWorldCovidData();
    // 提取所有日期
    this.dateList = worldData.map(item => item.date);
    // 如果有数据，则显示第一个日期的数据
    if (this.dateList.length > 0) {
